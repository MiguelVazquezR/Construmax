<template>
  <AppLayout title="Seguimiento de Correo">
    ...
    <div class="flex justify-between items-center">
      <span></span>
      <div class="mx-4 my-5">
        <Link v-if="this.$page.props.auth.user.permissions.includes('Enviar correos en seguimiento integral')"
          :href="route('crm.email-monitors.create')">
    ...
    <div v-if="currentTab == 1" class="md:grid grid-cols-2 border-b-2 border-[#cccccc] text-sm">
      <div class="grid grid-cols-2 text-left p-4 md:ml-10 border-r-2 border-gray-[#cccccc] items-center self-start">
        ...
        <a class="hover:underline text-primary"
          :href="route('crm.opportunities.show', email_monitor.data.opportunity?.id)">
        ...
        <a class="hover:underline text-primary"
          :href="route('users.show', email_monitor.data.seller?.id)">
      ...
      <div class="grid grid-cols-2 text-left p-4 md:ml-10 items-center">
        ...
        <a class="hover:underline text-primary"
          :href="route('crm.customers.show', email_monitor.data.customer?.id)">
        ...
        <span>{{ email_monitor.data.contact?.name }}</span>
