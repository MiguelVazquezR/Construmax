...
export default {
  data() {
    const form = useForm({
      ...
      tags: this.customer.tags.map(tag => tag.id),
    });
  ...
  methods: {
    update() {
      this.form.put(route('crm.customers.update', this.customer), {
        onSuccess: () => {
          this.$notify({
            title: 'Correcto',
  ...
  mounted() {
    this.form.contacts = this.customer.contacts.map(contact => {
      return {
        id: contact.id,
        name: contact.name,
        email: contact.email,
        ...
        branches: contact.additional ? contact.additional.branches : null
      };
    });
  }
};
