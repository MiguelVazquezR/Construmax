      ...
      <div class="mt-7 border-b-2">
        ...
        <div class="border-2">
          <div class="border-b-2 px-7 py-3 flex">
            <p @click="tabs = 1" :class="tabs == 1 ? 'bg-primarylight rounded-xl text-primary' : ''
              "
              class="h-10 p-2 cursor-pointer md:ml-5 transition duration-300 ease-in-out text-sm md:text-base leading-none">
              Roles
            </p>
            <div class="border-r-2 border-[#cccccc] h-10 ml-3"></div>
            <p @click="tabs = 2" :class="tabs == 2 ? 'bg-primarylight rounded-xl text-primary' : ''
          ...
          <div>
            <form @submit.prevent="editFlag ? updateRole() : storeRole()" ref="myRoleForm" class="grid grid-cols-3">
              <div class="col-span-full mb-4">
                ...
                <input v-model="roleForm.name" class="input" type="text">
          ...
          <div>
            <form @submit.prevent="editFlag ? updatePermission() : storePermission()" ref="myPermissionForm">
              <div>
                ...
                <input v-model="permissionForm.name" class="input" type="text">
                <InputError :message="permissionForm.errors.name" />
              </div>
              <div class="mt-3">
                <InputLabel value="Categoria del permiso *" class="ml-2" />
                <input v-model="permissionForm.category" class="input" type="text">
