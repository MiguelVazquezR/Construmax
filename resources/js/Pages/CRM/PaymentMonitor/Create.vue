<template>
    <AppLayout title="Registro de pago">
        <div class="flex justify-between items-center text-lg mx-8 mt-8">
            <b>Registrar pago o transacción</b>
            <Link
        ...
        <form @submit.prevent="store" class="mx-8 mt-3 grid grid-cols-2 gap-x-4 gap-y-2">
            ...
            <div class="w-full">
                ...
                <el-select disabled @change="cleanCustomerInfo" class="w-full" v-model="form.customer_id" clearable
                    filterable placeholder="Seleccione" no-data-text="No hay clientes registrados"
                    no-match-text="No se encontraron coincidencias">
...
export default {
    ...
    methods: {
        store() {
            this.form.post(route("crm.payment-monitors.store"), {
                onSuccess: () => {
                    this.$notify({
                        title: "Correcto",
                        message: "Se ha registrado un pago",
                        type: "success",
                    });
                },
            });
